...
def gossip_message(type, message):
    ...
    if not hasattr(config, "seen_messages"):
        config.seen_messages = set()
    message_id = (type, message)
    # if message_id in config.seen_messages:
    #     return
    config.seen_messages.add(message_id)

    for peer_id, peer_url in config.PEERS.items():
        if peer_id != config.node_id:
            send_message(peer_url, type, message)
