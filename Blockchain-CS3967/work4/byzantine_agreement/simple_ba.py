...
class SimplePKIBA:
    ...
    def calculate_votes_for(self, round):
        ...
        for proposal in self.get_proposals_with_threshold(round):
            if proposal not in self.s_i:
                ...
                my_sig = util.sign_message(proposal, config.SECRET_KEYS[config.node_id])
                if proposal not in self.signatures:
                    self.signatures[proposal] = []
                self.signatures[proposal].append((config.node_id, my_sig))

                if proposal not in self.votes:
                    self.votes[proposal] = set()
                self.votes[proposal].add(config.node_id)

                proposals_to_vote.append(proposal)

        return proposals_to_vote
    ...
    def get_proposals_with_threshold(self, round):
        ...
        proposals = []
        for proposal, voter_ids in self.votes.items():
            if len(voter_ids) >= round and len(voter_ids) > 0 and self.sender in voter_ids:
